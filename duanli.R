##阳性的对端粒/表型年龄
#将所有显著的cis暴露（eqtl，pqtl，100，90，99）移动到cisexp文件夹
...
dir.create("het")
dir.create("notfound")
setwd("/Users/hou/Desktop/cisexp") 
filename <- dir()
...
mediation_id <- c("ieu-b-4879","ebi-a-GCST90014288","ebi-a-GCST90014300","ebi-a-GCST90014292")
for(k in temp:length(filename))##此处是k，不能是i  length(filename)
{
  ...
  tryCatch({   #尝试提取结局，若因网络中断，则停止提取，输出暴露
    outcome_data <- extract_outcome_data(snps = exposure_data$SNP,outcomes = mediation_id)},
    error = function(e){
