...
outcome_gwas <- fread("Results_99th_percentile.txt",header = T,fill=T)   ##90还是99？
outcome_gwas <- outcome_gwas %>% rename(ID=SNP)  #把chr：pos命名为id

setwd("/Users/hou/Desktop/cis_pqtl_all/")  #cis_pqtl_all    pqtlexp
filename <- dir()
...
for(k in temp:length(filename))##此处是k，不能是i  length(filename)
{
  ...
  tryCatch({   #尝试提取结局，若因网络中断，则停止提取，输出暴露
    exposure_data$ID <- paste(exposure_data$chr.exposure,exposure_data$pos.exposure,sep = ":")
    expmatch <- exposure_data[,c("SNP","ID")]
    head(expmatch)
    outcome <- merge(expmatch,outcome_gwas,by="ID")  ##此处若等于true，则导致有些exp只有1个，然后本来匹配不上，却保留下来，导致后续error
    head(outcome)
    outcome_data <- format_data(
      dat = outcome,type = "outcome",snps = exposure_data$SNP,header = T,
      snp_col = "SNP",
      beta_col = "Beta",
      se_col = "SE",
      eaf_col = "EAF",
